...
int main() {
    std::cout << "=== Testing define, let, and set ===" << '\n' << '\n';

    // Test define
    std::cout << "--- DEFINE Tests ---" << '\n';
    test("Simple define", "(do "
                          "  (define-var x 42) "
                          "  x)");

    test("Define and use", "(do "
                           "  (define-var a 10) "
                           "  (define-var b 20) "
                           "  (+ a b))");
    ...
    test("Define returns value", "(define-var result 99)");
    ...
    test("Multiple bindings", "(let ((x 10) (y 20)) "
                              "  (+ x y))");

    test("Let with expressions", "(let ((x 5) (y (* 3 4))) "
                                 "  (+ x y))");

    test("Nested let", "(let ((x 10)) "
                       "  (let ((y 20)) "
                       "    (+ x y)))");

    test("Let shadows outer variable", "(do "
                                       "  (define x 10) "
                                       "  (let ((x 20)) "
                                       "    (print x) "
                                       "    x) "
    ...
    test("Let with multiple body expressions", "(let ((x 5)) "
                                               "  (print x) "
                                               "  (print (* x 2)) "
                                               "  (* x 3))");
    ...
    test("Factorial-like calculation", "(do "
                                       "  (define-var n 5) "
                                       "  (define-var result 1) "
                                       "  (set result (* result n)) "
                                       "  (set n (- n 1)) "
    ...
    test("Tagging with variables", "(do "
