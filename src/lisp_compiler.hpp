...
private:
    ...
    uint64_t* lookup_variable(const std::string& name) {
    ...
    void compile_expr(const ASTNodePtr& node) {
        switch (node->type) {
            ...
            case NodeType::SYMBOL: {
                ...
                uint64_t* addr = lookup_variable(sym);
                if (addr) {
                    // Load variable value
                    emit_opcode(Opcode::PUSH);
                    emit(*addr);
            ...
            case NodeType::LIST: {
                ...
                // Basic arithmetic operators
                    ...
                    if (items.size() < 3) throw std::runtime_error("+ requires at least 2 arguments");
                    ...
                    if (items.size() != 3) throw std::runtime_error("% requires exactly 2 arguments");
                    compile_expr(items[1]);
                ...
                // Control flow
                    ...
                    if (items.size() != 4) throw std::runtime_error("if requires 3 arguments: condition, then, else");
                ...
                // Sequential evaluation
                    ...
                    if (items.size() < 2) throw std::runtime_error("do requires at least 1 expression");
                ...
                // Print (for debugging)
                    ...
                    if (items.size() != 2) throw std::runtime_error("print requires exactly 1 argument");
                    compile_expr(items[1]);
                    emit_opcode(Opcode::PRINT);
                }
                else if (op == "print-string") {
                    if (items.size() != 2) throw std::runtime_error("print-string requires exactly 1 argument");
                    compile_expr(items[1]);
                    emit_opcode(Opcode::PRINT_STR);
                }
                // Bitwise operations
                else if (op == "bit-and") {
                    if (items.size() != 3) throw std::runtime_error("bit-and requires exactly 2 arguments");
                    compile_expr(items[1]);
                    compile_expr(items[2]);
                    emit_opcode(Opcode::AND);
                }
                else if (op == "bit-or") {
                    if (items.size() != 3) throw std::runtime_error("bit-or requires exactly 2 arguments");
                    compile_expr(items[1]);
                    compile_expr(items[2]);
                    emit_opcode(Opcode::OR);
                }
                else if (op == "bit-xor") {
                    if (items.size() != 3) throw std::runtime_error("bit-xor requires exactly 2 arguments");
                    compile_expr(items[1]);
                    compile_expr(items[2]);
                    emit_opcode(Opcode::XOR);
                }
                else if (op == "bit-shl") {
                    if (items.size() != 3) throw std::runtime_error("bit-shl requires exactly 2 arguments");
                    compile_expr(items[1]);
                    compile_expr(items[2]);
                    emit_opcode(Opcode::SHL);
                }
                else if (op == "bit-shr") {
                    if (items.size() != 3) throw std::runtime_error("bit-shr requires exactly 2 arguments");
                    compile_expr(items[1]);
                    compile_expr(items[2]);
                    emit_opcode(Opcode::SHR);
                }
                else if (op == "bit-ashr") {
                    if (items.size() != 3) throw std::runtime_error("bit-ashr requires exactly 2 arguments");
                    compile_expr(items[1]);
                    compile_expr(items[2]);
    ...
    void compile_define(const std::vector<ASTNodePtr>& items) {
        if (items.size() < 3) {
        ...
        if (items[1]->type == NodeType::LIST) {
            ...
            for (size_t i = 1; i < func_def.size(); i++) {
                if (func_def[i]->type != NodeType::SYMBOL) {
                    throw std::runtime_error("define: function parameters must be symbols");
                }
                params.push_back(func_def[i]->as_symbol());
            }
            
            // Function body is everything after the function definition
            if (items.size() != 3) {
                throw std::runtime_error("define: function requires exactly one body expression");
            }
            ...
            // Push nil as return value (define doesn't return anything useful for functions)
            emit_opcode(Opcode::PUSH);
            emit(0);
        ...
        // Define variable in current scope
        uint64_t addr = define_variable(var_name);
    ...
    void compile_set(const std::vector<ASTNodePtr>& items) {
        ...
        // Look up variable
        uint64_t* addr = lookup_variable(var_name);
        if (!addr) {
...
private:
    ...
    void compile_all_functions() {
        for (auto& [name, func] : functions) {
            ...
            // After CALL, stack is: [...] [arg1] [arg2] ... [argN] [ret_addr]
            // We need to pop ret_addr, then pop args, then push ret_addr back
            
            push_scope();
            
            // Pop and save return address
            // (We'll push it back before RET)
            // Actually, we can't easily "save" it without another stack
            // Let's use a temp variable
            uint64_t ret_addr_var = define_variable("__ret_addr__");
            emit_opcode(Opcode::PUSH);
            emit(ret_addr_var);
            emit_opcode(Opcode::STORE);
            ...
            for (const auto& param : func.params) {
                uint64_t addr = define_variable(param);
                param_addrs.push_back(addr);
            }
            
            // Pop each argument in reverse order (last param on top)
            ...
            // Now stack is: [result]
            // We need: [ret_addr] [result] for RET to work
            ...
            // Now stack is: [result] [ret_addr]
            // We need: [ret_addr] [result]
